...
fn main() {
    ...
    nes.load_rom("test_roms\\games\\donkey_kong.nes");
            ...
            match emu_mode {
                ...
                EmuMode::SingleFrame => {
                    if exec_frame {
                        ...
                        if enable_trace_log {
                            let cpu_log_file = File::create(format!("logs\\cpu_trace-frame-{}.log", nes.get_frame_number())).unwrap();
                            let mut cpu_log_writer = BufWriter::new(cpu_log_file);  
                            nes.output_cpu_log(&mut cpu_log_writer);

                            let ppu_log_file = File::create(format!("logs\\ppu_trace-frame-{}.log", nes.get_frame_number())).unwrap();
                            let mut ppu_log_writer = BufWriter::new(ppu_log_file);  
                            nes.output_ppu_log(&mut ppu_log_writer);
                        }
                    }
